...
function url_insert(&$set, $confirmer, $separateur) {
	...
	if (@sql_insertq('spip_urls', $set) <= 0) {
		...
		if (
			...
			sql_updateq('spip_urls', ['date' => date('Y-m-d H:i:s')], 'url=' . sql_quote($set['url']));
		}
		...
		do {
			...
			if (sql_countsel('spip_urls', $where  . sql_quote($set['url']))) {
				sql_updateq('spip_urls', ['url' => $set['url'], 'date' => date('Y-m-d H:i:s')], $where  . sql_quote($set['url']));
				...
				if (strlen($set['url']) > 200) {
					...
					sql_updateq('spip_urls', ['url' => $set['url'], 'date' => date('Y-m-d H:i:s')], 'url=' . sql_quote($set['url']));
					return true;
				}
			}
...
function url_verrouiller($objet, $id_objet, $url) {
	...
	sql_updateq('spip_urls', ['date' => date('Y-m-d H:i:s', time() + 10 * 365.25 * 24 * 3600)], $where);
}
